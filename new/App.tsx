  ...
  const processImportedCategories = (parsedData: any[]): MainCategory[] => {
    ...
    return parsedData.map((cat: any): MainCategory => {
      ...
      return {
        ...
        colorClasses: correspondingInitialCat ? correspondingInitialCat.colorClasses : { bg: 'bg-gray-50', border: 'border-gray-200', text: 'text-gray-500', hoverBorder: 'hover:border-gray-400' },
      };
    });
  };
  ...
  const renderContent = () => {
    ...
    return (
        <>
            ...
            {isEditing && <CategoryButton text="הוסף קטגוריה חדשה" icon={PlusIcon} onClick={() => handleAdd('main')} colorClasses={{ bg: 'bg-gray-100', border: 'border-gray-300', text: 'text-gray-500', hoverBorder: 'hover:border-gray-400' }} />}
            </div>
        </>
    )
  }
